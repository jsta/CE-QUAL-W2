...
!  calculating average violation concentration and writing to file
        ...
        open(CONE,file='envrprf_t.csv',status='unknown')
        ...
        open(CONE,file='envrprf_v.csv',status='unknown')
        ...
        open(CONE,file='envrprf_depth.csv',status='unknown')
...
if(nac_e > 0)then
       open(CONE,file='envrprf_c.csv',status='unknown')
...
4000 format(<nac_e>(a8,'_interval, Fraction_of_volume, '))
  ...
  if(nacd_e > 0)then     
       open(CONE,file='envrprf_cd.csv',status='unknown')
...
124       format(<nacd_e>(f6.2,',',e12.4,','))
125       format((f6.2,',',e12.4,','))
126       format(<nac_e>(f6.2,',',e12.4,','))
127       format(<nacd_e>(f6.2,',',e12.4,','))
