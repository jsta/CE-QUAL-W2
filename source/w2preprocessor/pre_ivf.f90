 ...
 If(file_exists) Then	
    ...
    !If(IncludeCEMAGenBODConstituents)Then
        ...
        IF (IncludeCEMAGenBODConstituents .and.  NumGenBODConstituents > NBOD) THEN
         CALL ERRORS
         WRITE (ERR,'(A,I4,A,I4)') 'Generic sediment BOD constituents are turned on but the number of generic sediment BOD constituents ',        NumGenBODConstituents,' > NBOD (the number of BOD compartments):',nbod
        END IF
        ...
        IF (IERR == 0) THEN
          ...
          DO WHILE (.TRUE.)
            ...
            IF(J>1)THEN
                ...
                ELSEIF(ABS(EDHO-EDH)/(JDAY-JDAYO) > 8.)THEN
                    CALL WARNINGS
                    WRITE (WRN,FMTF) 'Upstream Head BC: Water level change > 8 m/day on Julian date ',JDAY,' compared to previous date ',JDAYO,' in '//EDHFN(JB) 
                ENDIF
            ENDIF
...
!****** Constituent concentrations
        ...
        IF (CONSTITUENTS .AND. NAC > 0) THEN
          ...
          READ (NPT,'(/)')
            ...
            DO WHILE (.TRUE.)
                IF(CUHF==1)THEN
                READ (NPT,*,END=240,ERR=400)  JDAY,(XX,JAC=1,NAC)
                    !DO JAC=1,NAC
                    !CUH=XX(CN(JAC)) 
                    !ENDDO
                ELSE
                    ...
                    ELSE
                    READ (NPT,'(10F8.0:/(8X,9F8.0))',END=240,ERR=400) JDAY,(CUH,K=2,KB(US(JB)))
                    ENDIF
                    END DO
                END IF
          ...
          DO WHILE (.TRUE.)
           ! READ (NPT,'(10F8.0/8X,9F8.0)',END=250,ERR=400) JDAY, EDH
            ...
            IF(J>1)THEN
                ...
                ELSEIF(ABS(EDHO-EDH)/(JDAY-JDAYO) > 8.)THEN
                    CALL WARNINGS
                    WRITE (WRN,FMTF) 'Downstream Head BC: Water level change > 8 m/day on Julian date ',JDAY,' compared to previous date ',JDAYO,' in '//EDHFN(JB) 
                ENDIF
            ENDIF
...
!****** Constituent concentrations
        ...
        IF (CONSTITUENTS .AND. NAC > 0) THEN
          ...
          READ (NPT,'(/)')
            ...
            DO WHILE (.TRUE.)
                IF(CDHF==1)THEN
                READ (NPT,*,END=270,ERR=400)  JDAY,(XX,JAC=1,NAC)
                    !DO JAC=1,NAC
                    !CDH=XX(CN(JAC)) 
                    !ENDDO
                ELSE
                    DO JAC=1,NAC          
                    IF(CDHF==2)THEN
                    READ (NPT,*,END=270,ERR=400)  JDAY,(CDH,K=2,KB(DS(JB)))
                    ELSE
